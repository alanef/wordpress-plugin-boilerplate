...
def main():
    ...
    tests = [f for f in glob.glob(P("tests", "**", "*.php"), recursive=True)
             if os.path.basename(f).startswith("test-") or f.endswith("Test.php")]
    if not tests:
        write(P("tests", "test-plugin-load.php"), R("tests/test-plugin-load.php")); log("added smoke test tests/test-plugin-load.php")
